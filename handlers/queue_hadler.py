...
async def handle_join_command(websocket, message_text, user_id, group_id):
    ...
    if len(parts) == 2:
        ...
        if queue_name in ["0", "300", "600"]:
            response_message = join_queue(str(user_id), queue_name)
    ...
    queue_name = EloUtils.get_max_queue(PlayerUtils.get_elo(str(user_id)))
    join_queue(str(user_id), queue_name)
    return
...
async def handle_force_join_command(websocket, message_text, user_id, group_id):
    ...
    if len(parts) == 3:
        ...
        join_queue(str(user_id), queue_name, True)
        return